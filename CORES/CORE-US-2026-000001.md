...
Phone: 760-672-7323
PURL: https://tinmanmusicman-ops.github.io/ChatGPT/
PPIC:
...
On the technical side, builds structured, repeatable workflows using software, automation, and AI as accelerators—not autonomous decision-makers. Experienced in process design, file handling and cloud computing.
...
development tasks.
- implemented an audio Chat Bot using the PaymeGPT interface.
...
### PROJECT: Auto Gen of html from Excel SS
...
### PROJECT: Postnet to IMB barcode

- Situation:
The post office changed from PostNet barcode to intelligent mailbox code, causing delays in implementation.

- What I did:
I updated the customer forms to accommodate the new intelligent mailbox code and adjusted the presort job on the site to generate the correct barcode.

- Tools / systems:
Presort software, customer form templates, data import tools.  Mail Manager 2000

- Result:
Successfully created a new output file that included the updated presort information for our customers.

- Evidence / artifacts:
Updated customer forms, modified presort job configurations, new output file.

- Notes / caveats:
The project faced multiple deadline extensions from the post office, which required agile adjustments to our workflow.
...
- Situation:
Company records were maintained using paper-based tracking, limiting visibility into accounting accuracy, job tracking, and outstanding receivables.  Lack of structured data and consistent workflow made reporting and ongoing maintenance difficult.

- What I did:
Migrated paper-based records into QuickBooks to improve accounting accuracy and job tracking. Designed a consistent structure for customers, jobs, and categories to make data entry and reporting repeatable. Standardized workflow so ongoing updates did not depend on a single individual.
...
- Result:
Improved accounting accuracy and visibility into active and completed jobs.  Established repeatable bookkeeping and reporting process.  Recovered $17,000 in previously un-invoiced receivables after migration.

- Evidence / artifacts:
Operational QuickBooks records and structured customer/job database.  Verified recovery of outstanding receivables.

- Notes / caveats:
Migration performed from fully manual paper system into structured accounting platform.  Process improvements focused on accuracy, repeatability, and financial visibility.
...
- Situation:
Back-of-house dishwashing area was highly disorganized with no defined workflow, inconsistent methods, and large backlog of unprocessed dishes.  Lack of structure caused delays, inefficiency, and difficulty keeping up with kitchen demand in a high-volume environment.

- What I did:
Assessed workflow and identified bottlenecks causing backlog and inefficiency.  Designed and implemented a simple Standard Operating Procedure (SOP) to introduce structure and consistency.  Introduced categorization and staging method to sort dishware, utensils, and cookware by type.  Added pre-cleaning and preparation step to reduce wash time and improve throughput. Established production-line workflow enabling continuous, systematic processing instead of reactive pile-based cleaning.

- Tools / systems:
Commercial dishwashing station and sanitation workflow.  Manual SOP design and workflow structuring.  Staging, categorization, and process sequencing methods

- Result:
Reduced time required to clear dish backlog.  Improved overall kitchen support efficiency and workflow stability.  Enabled consistent, repeatable dishwashing process during high-volume operations.

- Evidence / artifacts:
Observed reduction in backlog and improved processing speed.  Team adoption of structured workflow and staging method.

- Notes / caveats:
Improvements implemented in a live operational environment with minimal tooling.  Demonstrated ability to rapidly analyze operational friction and implement practical process improvements in real-world conditions.
...
### SIGNALS:
- Managed printers and print jobs
- Promoted to department manager

### PROJECT: Automated Manual Presort and Paperwork Processing

- Situation:
The project aimed to automate the manual presort and paperwork processing for every manual job at Direct Mail of Maine, addressing the challenges posed by varying data formats and submission methods.

- What I did:
I developed a fully automated system that integrated with the existing postal shop software, creating a library and mechanisms to support the process while ensuring compliance with diverse standards.

- Tools / systems:
The project utilized the PostalSoft software and a custom-built automation framework to handle the different data formats and streamline the paperwork processing.

- Result:
The automation significantly improved efficiency in processing manual jobs, reduced errors, and allowed the department to operate with limited resources more effectively.

- Evidence / artifacts:
Documentation of the automation process, system architecture diagrams, and performance metrics demonstrating the improvements in processing time and accuracy.

- Notes / caveats:
The project faced challenges in standardizing data formats and required ongoing adjustments to accommodate the various ways data was submitted.

### PROJECT: Postnet to IMB Barcode Migration

- Situation:
The project involved converting all pre-sorting jobs in print jobs from using the PostNet barcode to the Intelligent Mail barcode due to changes in the post office requirements.

- What I did:
I updated all of the presorting information, modified the insert launch, and adjusted every job to generate the new Intelligent Mail barcode and the corresponding paperwork.

- Tools / systems:
The project utilized various print job management systems and automation tools to handle the modifications and updates required for the new barcode system.

- Result:
The successful implementation of the new Intelligent Mail barcode across all pre-sorting jobs, ensuring compliance with the updated post office requirements.

- Evidence / artifacts:
Documentation of the updated presorting information, modified job specifications, and correspondence with the post office regarding the implementation process.

- Notes / caveats:
The project faced delays due to the post office's implementation timeline, which impacted our migration path and required additional coordination for sign-offs and verifications.
