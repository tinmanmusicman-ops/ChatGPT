...
def _search_uid_in_folder(mail: imaplib.IMAP4_SSL, folder: str, sender: str, unseen_only: bool = False,use_category: bool = False, category: Optional[str] = None) -> Optional[bytes]:
...
def _fetch_body_by_uid(mail: imaplib.IMAP4_SSL, uid: bytes, mark_read: bool = True) -> str:
...
def fetch_latest_email_body_multi(user: str, app_password: str, folder: str,
    ...
    def _consider(uid: Optional[bytes], label: str, s: str):
        ...
        candidates.append((uid, internal_date))
    ...
    best_uid, _ = max(candidates, key=lambda tup: _parse_dt(tup[1]))
    body = _fetch_body_by_uid(mail, best_uid, mark_read=mark_read)
...
def main():
    ...
    gmail_only_unseen = bool(cfg.get("gmail_only_unseen", False))
    ...
    body = fetch_latest_email_body_multi(
        gmail_user,
        gmail_app_password,
        folder=gmail_folder,
        senders=senders,
