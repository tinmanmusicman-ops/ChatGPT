#!/usr/bin/env python3
"""
poll_indeed_from_sender.py
- Polls Gmail for messages FROM a specific sender (Indeed)
- Appends only [Timestamp, Message Text] to a Google Sheet tab named "Indeed"
- Cleans text (HTML -> plain text, removes boilerplate, footers, quotes)
...
# ------------ Main ------------
def main():
    # Force working directory to script folder (consistent with your framework)
    ...
    log.info("Starting Indeed poller (from=%s | max=%s | unseen_only=%s | mark_read=%s | thread=%s)",
