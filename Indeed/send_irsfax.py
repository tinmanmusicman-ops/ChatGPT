...
def resolve_config_path(explicit: Optional[str] = None) -> Path:
    ...
    candidates = [
        base / "config.json",
        base / "Indeed" / "config.json",
        base.parent / "config.json",
        base.parent / "Indeed" / "config.json",
    ]
    for candidate in candidates:
        if candidate.exists():
            return candidate
    raise FileNotFoundError(
        "No config file found. Please pass --config or create Indeed/config.json."
    )
...
def normalize_credentials(cfg: dict) -> None:
    if not cfg.get("email_user") and cfg.get("user"):
        cfg["email_user"] = cfg["user"]
    if not cfg.get("email_password") and cfg.get("app_password"):
