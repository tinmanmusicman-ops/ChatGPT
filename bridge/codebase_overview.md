...
- **bridge/** – Collaboration handoffs (this overview lives here). The folder name is lowercase `bridge/` at the repository root. The files are committed on the local `work` branch, and no Git remote is configured yet. You can inspect them with `git show work:bridge/README.md` and `git show work:bridge/codebase_overview.md`. If you need them on GitHub, add a remote and push `work`.
