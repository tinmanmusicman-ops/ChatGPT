...
def build_dashboard_html(
    ...
    <script>
      ...
      const handsLogo = document.createElement("div");
      handsLogo.className = "hands-logo";
      chartControls?.appendChild(handsLogo);
      const chartLogo = document.createElement("div");
      chartLogo.className = "chart-logo";
      chartWrap?.appendChild(chartLogo);
      ...
      const modeOrder = ["setpoint", "actual", "outside", "cooling", "fan"];
      ...
      const activeModes = new Set(modeOrder);
      const updateLegendEntries = () => {{
        if (!legendEntry) {{
          return;
        }}
        const entries = [];
        activeModes.forEach((mode) => {{
          const values = legendMap[mode];
      ...
      const updateAxesVisibility = () => {{
        const showTemp =
          activeModes.has("setpoint") ||
          activeModes.has("actual") ||
          activeModes.has("outside");
        const showFan = activeModes.has("fan");
        const showCooling = activeModes.has("cooling");
      ...
      const updateChartVisibility = () => {{
        modeOrder.forEach((mode) => {{
          const datasetIndex = datasetIndexByMode[mode];
          if (chart && typeof datasetIndex === "number") {{
            chart.data.datasets[datasetIndex].hidden = !activeModes.has(mode);
          }}
        }});
      }};
      ...
      const toggleMode = (mode) => {{
        if (mode === "both") {{
          modeOrder.forEach((m) => activeModes.add(m));
        }} else if (mode) {{
      ...
      const showChart = () => {{
        ...
        canvas.style.display = "block";
        if (chartControls) {{
          chartControls.classList.add("hands-visible");
        }}
      ...
      const hideChart = () => {{
        ...
        if (chartControls) {{
          chartControls.classList.remove("hands-visible");
        }}
      }};
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      .chart-controls.hands-visible .hands-logo {{
        opacity: 1;
      ...
      .chart-wrap.logo-top-right .chart-logo {{
        opacity: 1;
        transform: translateY(0);
      }}
      .fan-legend-image {{
