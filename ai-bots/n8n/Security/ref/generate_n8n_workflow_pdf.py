...
def generate_flowchart_image(workflow: dict[str, Any]) -> None:
    ...
    fig = plt.figure(figsize=(16, 9), dpi=150, facecolor="#0b0f14")
    ...
    ax.axis("off")
    ax.set_title(
        "n8n Security Gmail to Sheets Poller - Flowchart",
        fontsize=16,
        pad=14,
        color="#e6edf3",
    )
    ...
    for src, dst, branch in edges:
        ...
        color = "#58a6ff" if branch == 0 else "#ff7b72"
        ax.annotate(
            "",
            xy=(dp[0], dp[1]),
            xytext=(sp[0], sp[1]),
            arrowprops=dict(arrowstyle="->", color=color, lw=1.4, alpha=0.95),
            zorder=1,
        )
