...
def search_unseen_uids(imap: imaplib.IMAP4_SSL, subject_term: str, include_all_mail: bool = True) -> List[bytes]:
    """
    Return UIDs for UNSEEN messages matching subject_term.
...
def fetch_headers_peek(imap: imaplib.IMAP4_SSL, uid: bytes) -> Tuple[str, str, Optional[str]]:
    """
    Returns (subject, from_, message_id). Uses BODY.PEEK to avoid flipping \Seen.
    """
    typ, data = imap.uid("FETCH", uid, "(BODY.PEEK[HEADER.FIELDS (SUBJECT FROM MESSAGE-ID)])")
...
def fetch_gm_msgid(imap: imaplib.IMAP4_SSL, uid: bytes) -> Optional[str]:
    ...
    if typ != "OK" or not data or not data[0]:
        return None
    # Example response part: b'123 (X-GM-MSGID 1707412345678901234 UID 12345)'
    part = data[0][0] if isinstance(data[0], tuple) else data[0]
    if isinstance(part, bytes):
        part = part.decode("utf-8", "ignore")
    import re as _re
    m = _re.search(r"X-GM-MSGID\s+(\d+)", part)
