...
class ThemePreviewWindow(tk.Toplevel):
    """Standalone window that renders the live theme preview."""

    def __init__(self, parent: tk.Toplevel, editor: "ConfigEditor"):
        ...
        preview_group.grid(row=0, column=0, sticky="nsew", padx=12, pady=12)
        ...
        self.preview_card.grid(row=0, column=0, sticky="nsew", pady=(6, 12))
...
class ThemeColorPicker(tk.Toplevel):
    ...
    def __init__(self, editor: "ConfigEditor", base_theme: str):
        ...
        self._picker_container = container
        self.rowconfigure(0, weight=1)
        ...
        self._scroll_canvas_window = self._scroll_canvas.create_window((0, 0), window=scroll_frame, anchor="nw")
        scroll_frame.bind("<Configure>", self._on_scroll_frame_configure)
        ...
        colors_group.columnconfigure(0, weight=1)
        for idx, (label, key) in enumerate(self.color_fields):
            row = ttk.Frame(colors_group)
            row.grid(row=idx, column=0, sticky="ew", pady=(4, 8))
        ...
        self.preview_window = ThemePreviewWindow(self, editor)
        self.preview_window.protocol("WM_DELETE_WINDOW", lambda: None)
        ...
        self.bind("<Destroy>", self._on_picker_destroy)
        self.after(0, self._fit_to_content)
        self.after(0, self._position_preview_window)
    ...
    def _position_preview_window(self):
        ...
        picker_w = self.winfo_width()
        picker_x = self.winfo_x()
        picker_y = self.winfo_y()
        preview_w = preview.winfo_width()
        preview_h = preview.winfo_height()

        space_right = screen_w - (picker_x + picker_w)
        space_left = picker_x
        if space_right >= preview_w + 12:
            target_x = picker_x + picker_w + 12
        else:
            target_x = picker_x - preview_w - 12
        x = min(screen_w - preview_w - 10, max(10, target_x))
        y = min(max(10, picker_y), screen_h - preview_h - 10)
        preview.geometry(f"+{int(x)}+{int(y)}")

    def _fit_to_content(self):
        container = getattr(self, "_picker_container", None)
        if not container:
            return
        self.update_idletasks()
        target_width = container.winfo_reqwidth() + 24
        target_height = max(560, self.winfo_height())
        self.geometry(f"{int(target_width)}x{int(target_height)}")
        self.minsize(int(target_width), 560)

    def _on_picker_destroy(self, event):
    ...
    def _on_scroll_canvas_configure(self, event):
        if not hasattr(self, "_scroll_canvas_window"):
            return
